/** hadoop-20 / src / hdfs / org / apache / hadoop / hdfs / server / namenode / 
***
*** Hadoop - HDFS Custom Block Placement Policy
*************************************************************************************
 * Modification and utilisation of this code is Licensed to one or more contributor agreements.
 *     http://www.apache.org/licenses/LICENSE-2.0
 * See the License for the specific language governing permissions and
 * limitations under the License.
 * 25 between classes, procedures and functions
 */
...
// a fair RT Reference Table values lock for File reader.
// block's file name must be the same as in dfs CopyFromLocal during the implementation 
  
    protected class ReadblockInfo {
    public int index;
    public List<String> blockid;
    public HashMap<String,Integer> ReadBlockTable;

    public Integer findblockRT(DatanodeDescriptor node) {
    Integer retVal = ReadblockInfo.get(RT.getBlockName());
	for (int in2 = 0; in2 < listOfNodes.size(); in2++) {
      DatanodeDescriptor n2 = listOfNodes.get(in2);
      if (!first.equals(n2)) {
        if (result[1] == null && inWindow(first, n2)) {
          result[0] = RT.getBlockRef(Ref);
          result[1] = RT.getBlockName(in2);
        }
      ...
      if (retVal == null) {
        retVal = rackNodesMap.get(block.getName());
        if (retVal == null) {
          retVal = rackNodesMap.get(block.getHost());
          if (retVal == null) {
            LOG.info("block not find " + block.getHostName() +
                " - " + node.getName() + " - " + node.getHost());
          }
        }
      }
